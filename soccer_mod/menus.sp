...
public void OpenMenuCredits(int client)
{
	...
	menu.AddItem("groupcssl", "CSSL Group");
	menu.AddItem("websitecssl", "CSSL Website");
	menu.AddItem("discordcssl", "CSSL Discord");
...
public int MenuHandlerCredits(Menu menu, MenuAction action, int client, int choice)
{
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(menuItem, "groupcssl"))   CPrintToChat(client, "{%s}[%s] {%s}https://steamcommunity.com/groups/cssleu", prefixcolor, prefix, textcolor);
		else if (StrEqual(menuItem, "websitecssl"))   CPrintToChat(client, "{%s}[%s] {%s}www.cssleague.eu", prefixcolor, prefix, textcolor);
		else if (StrEqual(menuItem, "discordcssl"))   CPrintToChat(client, "{%s}[%s] {%s}https://discord.gg/4AqP8ypzn9", prefixcolor, prefix, textcolor);
