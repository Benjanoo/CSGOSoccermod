public void OpenInfoPanel(int client)
{
	...
	int cStatus = '-';
	char sItem_display[6][32] = {"Messages",
	"Progress Bar", "Sound", "Timer", "Timer Settings", "Close"};
	...
	for(int i = 1; i <= 6; i++)
	{
		...
		if(i == 6)  Format(sBuf, sizeof(sBuf), "->%i. %s", i, sItem_display[(i-1)]);
		DrawPanelText(h_Infopanel, sBuf);
	}
...
public int InfoPanelReturn(Handle panel, MenuAction action, int client, int key)
{
	...
	else if(action == MenuAction_Select && key == 6)	
	{
